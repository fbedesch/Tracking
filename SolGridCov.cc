...
SolGridCov::SolGridCov()
{
	...
	//
	fNminHits = 6;	// Set default acceptance hits
//
}
//
// Destructor
...
void SolGridCov::Write(TString fname, SolGeom *G)
{
	...
	//
	// Write acceptance and close file
	fAcc->WriteAcceptance(fout);
	fout->Close();
	delete fout;
}
...
void SolGridCov::Read(TString fname)
{
	...
	delete f;
	//
	// Read acceptance if available
	fAcc = new AcceptanceClx(fname);
	//
}
//
