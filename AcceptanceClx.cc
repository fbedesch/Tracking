...
AcceptanceClx::AcceptanceClx(SolGeom* InGeo)
{
		...
		for (Int_t ipt = 0; ipt < NpPt - 1; ipt++)	// Scan pt bins
		{
			for (Int_t ith = 0; ith < NpTh - 1; ith++)	// Scan theta bins
			{
				...
				if (dPt > dPtMin && dAp > dAmin && Nsplits < MaxSplits) {
					//cout << "Pt(" << ipt << ") = " << Pta(ipt) << ", dAp = " << dAp << endl;
					NsplitCnt++;	// Total splits counter
					Nsplits++;	// Increase splits/cycle
					NpPt++;		// Increase #pt points 
					//cout << "New pt split: dAp = " << dAp << ", dPt = " << dPt <<
					//	", Nsplits = " << Nsplits << ", NpPt = " << NpPt << endl;
				...
				if (dTh > dThMin && dAt > dAmin && Nsplits < MaxSplits) {
					...
					//cout << "New th split: dAt = " << dAt << ", dTh = " << dTh <<
					//	", Nsplits = " << Nsplits << ", NpTh = " << NpTh << endl;
	...
	//
	cout<<"AcceptanceClx:: Acceptance generation completed after "<<Ncycles<<" cycles"
		<<" and a total number of "<<NsplitCnt<< " splits"<<endl;
	//
	...
	if (PtCk != NpPt)
		cout << "AcceptanceClx:: Error in grid generation NpPt=" << NpPt << ", Pta size= " << PtCk << endl;
	...
	if (ThCk != NpTh)
		cout << "AcceptanceClx:: Error in grid generation NpTh=" << NpTh << ", Tha size= " << ThCk << endl;
	fThArray.ResizeTo(NpTh);
	fThArray = Tha;	// Array of Theta nodes
	//
	cout << "AcceptanceClx:: Acceptance encoding with " << fNPtNodes
		<<" pt nodes and "<< fNThNodes <<" theta nodes"<< endl;
	Int_t Nrows = fAcc.GetNrows();
	Int_t Ncols = fAcc.GetNcols();
	//cout<<"AcceptanceClx:: fAcc rows: "<<Nrows<<", cols: "<<Ncols<<endl;
	//cout<<"AcceptanceClx:: Pta size: "<<Pta.GetNrows()<<", Tha size: "<<Tha.GetNrows()<<endl;;
	//cout<<"AcceptanceClx:: pt nodes"<<endl; fPtArray.Print();
	//cout<<"AcceptanceClx:: th nodes"<<endl; fThArray.Print();
}
...
void AcceptanceClx::ReadAcceptance(TString InFile)
{
	...
	fThArray = *pThArray;
	//
	cout << "AcceptanceClx::Read complete: Npt= " << fNPtNodes << ", Nth= " << fNThNodes << endl;
	//
	f->Close();
	delete f;
}
...
Double_t AcceptanceClx::HitNumber(Double_t pt, Double_t theta)
{
	...
	//cout << "AcceptanceClx::HitNumber: just in pt= " << pt << ", theta = " << theta << endl;
	//cout << "AcceptanceClx::HitNumber: ptArr(0)= " << fPtArray(0) << ", thArr(0) = " << fThArray(0) << endl;
	//cout << "AcceptanceClx::HitNumber: ptArr(E)= " << fPtArray(fNPtNodes - 1) 
	//	<< ", thArr(E) = " << fThArray(fNThNodes - 1) << endl;
	Float_t eps = 1.0e-4;
	...
	{
		cout << "Search error: (ip, pt) = (" << ip << ", " << pt << "), pt0 = " << pt0 << endl;
		cout << "Search error: pt nodes = " << fNPtNodes 
			<< " , last value = " << fPtArray(fNPtNodes - 1) << endl;
	}
	if (it<0 || ip >fNThNodes - 2)
	{
		cout << "Search error: (it, th) = (" << it << ", " << theta << "), th0 = " << th0 << endl;
		cout << "Search error: th nodes = " << fNThNodes
			<< " , last value = " << fThArray(fNThNodes - 1) << endl;
	}
	//
